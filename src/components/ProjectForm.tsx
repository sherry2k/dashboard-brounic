...
export default function ProjectForm({
  ...
  const [shopDrawingsStatus, setShopDrawingsStatus] = useState(
    project?.shopDrawingsStatus ?? "not_started"
  );
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        {type === "supply" && (
          <div className="sm:col-span-2">
            ...
            <select
              value={shopDrawingsStatus}
              onChange={(e) => setShopDrawingsStatus(e.target.value)}
              className={inputCls}
            >
              {SHOP_DRAWINGS_STATUSES.map((s) => (
                <option key={s} value={s}>
                  {statusLabel(s)}
                </option>
              ))}
